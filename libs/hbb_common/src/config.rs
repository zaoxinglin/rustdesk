...
// [https://github.com/quic-go/quic-go/issues/525#issuecomment-294531351](https://github.com/quic-go/quic-go/issues/525#issuecomment-294531351)
...
// [https://www.onsip.com/voip-resources/voip-fundamentals/what-is-nat-keepalive](https://www.onsip.com/voip-resources/voip-fundamentals/what-is-nat-keepalive)
...
pub const RENDEZVOUS_SERVERS: &[&str] = &["223.100.77.206:21116"];
pub const RS_PUB_KEY: &str = "qzcMRw+ujkXP+e9Lge8s0KJNLqyxDz9dLXXl8zV4PJI=";
...
postfix == "_service" || postfix.starts_with("*uinput*")
}
...
.into();
#[cfg(target_os = "macos")]
return *tmp.replace("Application Support", "Preferences").into();
#[cfg(target_os = "linux")]
{
if *tmp == "/root" {
...
"base64*".to_string() + base64::encode(id, base64::Variant::Original).as_str()
} else {
id.to_string()
};
...
path = [PEERS, id.replace(":", "*").as_str()].iter().collect();
}
...
.unwrap_or("")
.to_owned();
let id_decoded_string = if id.starts_with("base64*") && id.len() != 7 {
let id_decoded =
...
let mut futs = vec![];
for (*, *, path) in vec_id_modified_time_path.into_iter() {
...
let peers: Vec<*> = all[from..to]
.iter()
.map(|(id, t, p)| {
let c = PeerConfig::load(&id);
if [c.info](https://c.info).platform.is_empty() {
fs::remove_file(p).ok();
}
(id.clone(), t.clone(), c)
})
.filter(|p| ![p.2.info](https://p.2.info).platform.is_empty())
.collect();
(peers, to)
}
...
impl LocalConfig {
fn load() -> LocalConfig {
let mut config = Config::load*::<LocalConfig>("*local");
// 首次启动默认关闭更新检查、禁止自动更新
...
let *lock = CONFIG.read().unwrap();
match confy::load_path(Config::file*("*lan_peers")) {
Ok(peers) => peers,
Err(err) => {
...
Config::load*::<UserDefaultConfig>("*default")
}
#[inline]
fn store(&self) {
Config::store*(self, "_default");
}
...
self.get_string(key, "best", vec!["best", "low", "custom"])
}
keys::OPTION_CODEC_PREFERENCE => {
...
> impl Status {
> fn load() -> Status {
> Config::load_::<Status>("*status")
> }
> fn store(&self) {
> Config::store*(self, "_status");
> }
...
let *guard = CONFIG_STATE_TEST_LOCK.lock().unwrap();
let *file_guard = ConfigFileRestoreGuard::new(Config::file*("2"));
let pin = "123456";
...
use std::os::unix::fs::PermissionsExt;
assert_eq!(
// ignore file type information by masking with 0o777 (see [https://stackoverflow.com/a/50045872](https://stackoverflow.com/a/50045872))
